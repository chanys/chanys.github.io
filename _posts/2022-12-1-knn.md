...
### Norms
* **L2 norm**: Given a vector $\textbf{x} = [x_1, \ldots, x_n]$, its length ($L^2$ norm) $\lVert \textbf{x} \rVert_{2} = \sqrt{\sum_{i} x_i^{2}}$

### Computation Efficient Distance Calculation
* For distance between vectors $\textbf{x}$ and $\textbf{y}$, we can use Euclidean distance. For computation efficency, we often calculate the Squared Euclidean distance: $\lVert \textbf{x} - \textbf{y} \rVert_2^2 = \sum_{i} (x_i - y_i)^2$. 
	...
	* For computation efficiency, we can use matrix multiplication by simplifying $\sum_{i} (x_i - y_i)^2$ as follows: 
    ...
    $$= \textbf{x} \cdot \textbf{x} + \textbf{y} \cdot \textbf{y} - 2 (\textbf{x} \cdot \textbf{y}) = \lVert \textbf{x} \rVert_2^2 + \lVert \textbf{y} \rVert_2^2 - 2(\textbf{x} \cdot \textbf{y})$$
